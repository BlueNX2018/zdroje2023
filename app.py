    ...
    if not login or not haslo:
        ...
        try:
            ...
            if not rows:
                ...
                #st.download_button(
                #    label="Pobierz CSV z wiadomościami",
                #    data=csv,
                #    file_name=f"maile_wyslane_{selected_date}_{start_time}_{end_time}.csv",
                #    mime="text/csv",
                #)
                ...
                #st.write(f"OK alias: **{ok_alias_count}**")
                #st.write(f"OK z błędem: **{ok_error_count}**")
                #st.write(f"DO WERYFIKACJI: **{review_count}**")
                #st.write(f"BRAK: **{missing_count}**")
            ...
            report_csv = report_df.to_csv(index=False).encode("utf-8-sig")

                
                # st.download_button(
                #     label="Pobierz CSV z raportem braków",
                #     data=report_csv,
                #     file_name=f"raport_brakow_{selected_date}_{start_time}_{end_time}.csv",
                #     mime="text/csv",
                # )
